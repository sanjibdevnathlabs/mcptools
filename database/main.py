...
from database.src.logging_config import generate_trace_id, get_logger, setup_logging
...
Examples:
  ...
  %(prog)s --config-check                       # Validate configuration only
    ...
    parser.add_argument(
        "--test-query", type=str, help="Execute a specific SQL query in test mode"
    )

    # Server options (for SSE mode)
    parser.add_argument(
        "--host", default=None, help="Host to bind to for SSE mode (overrides config)"
    )
    ...
    # Debug and logging
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode (overrides config)"
    )
    ...
    # Version
    parser.add_argument(
        "--version", action="version", version="Database MCP Server 1.0.0"
    )
            ...
            try:
                ...
                if execute_tool:
                    ...
                    result = await server.mcp.call_tool(
                        execute_tool.name, {"sql": test_query}
                    )
...
async def run_server(
    ...
    try:
        ...
        logger.info(
            f"SERVER_STARTING: trace_id={trace_id}, transport={config.server.transport_mode}, db={config.database.host}:{config.database.port}"
        )
