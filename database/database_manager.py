...
class DatabaseManager:
    ...
    async def initialize_pool(self) -> None:
        ...
        if self.pool:
            self.logger.warning("POOL_ALREADY_INITIALIZED", {"dsn": self.config.get_database_dsn()})
            return
        
        try:
            connection_params = self.config.get_connection_params()
            self.logger.info("POOL_INIT_START", {
            ...
            self.logger.info("POOL_INIT_SUCCESS", {
                "dsn": self.config.get_database_dsn(),
            ...
            self.logger.error("POOL_INIT_FAILED", {
                "error": str(e),
    ...
    async def _test_connection(self) -> None:
        ...
        try:
            ...
            self.logger.info("CONNECTION_TEST_SUCCESS", {
                "dsn": self.config.get_database_dsn()
            })
        except Exception as e:
    ...
    async def close_pool(self) -> None:
        ...
        if self.pool:
            try:
                ...
                self.logger.info("POOL_CLOSE_SUCCESS", {
                    "dsn": self.config.get_database_dsn()
                })
                self.pool = None
            except Exception as e:
                self.logger.error("POOL_CLOSE_FAILED", {
                    "error": str(e),
                    "error_type": type(e).__name__,
                    "dsn": self.config.get_database_dsn()
                }, exc_info=True)
    ...
    async def execute_query(
        ...
        self.logger.info("QUERY_EXECUTE_START", {
            "query_id": query_id,
        ...
        try:
            async with self.get_connection() as conn:
                async with conn.cursor(aiomysql.DictCursor) as cursor:
                    ...
                    try:
                        await cursor.execute(f"SET SESSION max_execution_time = {int(self.config.database.query_timeout * 1000)}")
                    ...
                    if cursor.description:
                        ...
                        if fetch_results:
                            ...
                            results = [dict(row) for row in results]  # Convert to regular dicts
                    else:
                    ...
                    self.logger.info("QUERY_EXECUTE_SUCCESS", {
                        "query_id": query_id,
                    ...
                    response = {
                        ...
                        "metadata": {
                            ...
                            "affected_rows": affected_rows,
                            "has_results": bool(cursor.description),
                            "more_rows_available": more_rows_available if 'more_rows_available' in locals() else False
                        }
                    }
            ...
            self.logger.error("QUERY_MYSQL_ERROR", {
                "query_id": query_id,
                "execution_time": execution_time,
                "error": str(e),
                "error_type": type(e).__name__,
                "mysql_errno": getattr(e, 'args', [None])[0] if hasattr(e, 'args') else None,
                "sql_preview": sql[:100] + ('...' if len(sql) > 100 else '')
            }, exc_info=True)
            ...
            try:
                ...
                self.logger.error("QUERY_ROLLBACK_FAILED", {
                    "query_id": query_id,
            ...
            self.logger.error("QUERY_UNEXPECTED_ERROR", {
                "query_id": query_id,
                "execution_time": execution_time,
                "error": str(e),
                "error_type": type(e).__name__,
    ...
    async def get_schema_info(self, database_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("SCHEMA_INFO_FAILED", {
                "error": str(e),
                "error_type": type(e).__name__,
    ...
    async def get_table_info(self, table_name: str, database_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("TABLE_INFO_FAILED", {
                "error": str(e),
                "error_type": type(e).__name__,
    ...
    async def explain_query(self, sql: str, params: Optional[List[Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("EXPLAIN_QUERY_FAILED", {
                "error": str(e),
                "error_type": type(e).__name__,
                "sql_preview": sql[:100] + ('...' if len(sql) > 100 else '')
            }, exc_info=True)
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            start_time = time.time()
            result = await self.execute_query("SELECT 1 as health_check, NOW() as server_time")
            ...
            return {
                ...
                "database_time": result.get("data", [{}])[0].get("server_time") if result.get("success") else None,
