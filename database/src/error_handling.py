            ...
            response.update(
                {
                    ...
                    "recovery_action": (
                        error_context.recovery_action.value
...
class GracefulDegradationManager:
    ...
    def set_service_state(self, service_name: str, state: ServiceState) -> None:
        ...
        if (
            state in [ServiceState.DEGRADED, ServiceState.UNAVAILABLE]
            and old_state == ServiceState.AVAILABLE
        ):
    ...
    def get_cached_response(
        self, cache_key: str, max_age: float = 300.0
...
class ErrorHandler:
    ...
    async def handle_operation(
        ...
        try:
            ...
            if service_name and isinstance(
                e, DatabaseConnectionError | ResourceExhaustionError
            ):
                self.degradation_manager.set_service_state(
                    service_name, ServiceState.DEGRADED
                )
    ...
    def format_error_response(
        ...
        return self.error_formatter.format_error_response(
            error, error_context, include_stack_trace=include_debug_info
        )
