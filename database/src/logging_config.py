...
def setup_logging(config) -> dict[str, TraceCodeLogger]:
    ...
    if config.server.log_format == "json":
        ...
        formatter = TextFormatter(include_trace_id=config.server.log_include_trace_id)
    ...
    loggers = {
        ...
        "server": TraceCodeLogger(
            "database.server", logging.getLogger("database.server")
        ),
        "database": TraceCodeLogger(
            "database.manager", logging.getLogger("database.manager")
        ),
        "security": TraceCodeLogger(
            "database.security", logging.getLogger("database.security")
        ),
        "transport": TraceCodeLogger(
            "database.transport", logging.getLogger("database.transport")
        ),
        "config": TraceCodeLogger(
            "database.config", logging.getLogger("database.config")
        ),
        "schema": TraceCodeLogger(
            "database.schema", logging.getLogger("database.schema")
        ),
        "monitoring": TraceCodeLogger(
            "database.monitoring", logging.getLogger("database.monitoring")
        ),
    }
    ...
    config_logger.info(
        "LOGGING_CONFIGURED",
        {
            ...
            "log_file_path": (
                str(config.server.log_file_path)
                if config.server.log_destination in ("file", "both")
                else None
            ),
        },
