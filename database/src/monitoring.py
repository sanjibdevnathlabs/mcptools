...
class PerformanceTracker:
    ...
    def record_connection_event(
        self, event_type: str, client_id: str, details: dict[str, Any] = None
    ) -> None:
    ...
    def update_peak_connections(self, current_connections: int) -> None:
        ...
        with self._lock:
            if (
                current_connections
                > self.peak_usage_tracker["max_concurrent_connections"]
            ):
                self.peak_usage_tracker["max_concurrent_connections"] = (
                    current_connections
                )
                self.peak_usage_tracker["peak_timestamps"][
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        with self._lock:
            now = time.time()
            recent_queries = [
                q for q in self.query_times if now - q["timestamp"] < 300
            ]  # Last 5 minutes

            if recent_queries:
                avg_query_time = sum(q["execution_time"] for q in recent_queries) / len(
                    recent_queries
                )
                success_rate = sum(1 for q in recent_queries if q["success"]) / len(
                    recent_queries
                )
            else:
                avg_query_time = 0.0
            ...
            return {
                ...
                "recent_connection_events": list(self.connection_events)[
                    -10:
                ],  # Last 10 events
                "slow_queries_sample": list(self.slow_queries)[
                    -5:
                ],  # Last 5 slow queries
            }
...
class ErrorTracker:
    ...
    def record_error(
        ...
        with self._lock:
            ...
            self.error_rates[error_type] = [
                bucket for bucket in self.error_rates[error_type] if bucket > cutoff
            ]
    ...
    def get_error_summary(self) -> dict[str, Any]:
        ...
        with self._lock:
            now = time.time()
            recent_errors = [
                e for e in self.errors if now - e.timestamp < 3600
            ]  # Last hour
...
class HealthChecker:
    ...
    async def _check_connection_pool(self) -> HealthCheckResult:
        ...
        try:
            ...
            utilization = (
                (pool_size - pool_free_size) / pool_max_size if pool_max_size > 0 else 0
            )

            if utilization > 0.9:
                status = HealthStatus.CRITICAL
                message = (
                    f"Connection pool critically full ({utilization:.1%} utilization)"
                )
    ...
    async def _check_system_resources(self) -> HealthCheckResult:
        ...
        try:
            ...
            return HealthCheckResult(
                name="system_resources", status=status, message=message, details=metrics
            )
        except Exception as e:
            return HealthCheckResult(
                name="system_resources",
...
class ProductionMonitor:
    ...
    def record_query_metrics(
        ...
        self.performance_tracker.record_query_execution(
            query_type, execution_time, success, client_id
        )
    ...
    async def get_comprehensive_status(self) -> dict[str, Any]:
        ...
        with self._metrics_lock:
            recent_metrics = [
                m for m in self.metrics if timestamp - m.timestamp < 300
            ]  # Last 5 minutes

        return {
            "timestamp": timestamp,
            "uptime": timestamp
            - performance_metrics.get("peak_usage", {})
            .get("peak_timestamps", {})
            .get("max_concurrent_connections", timestamp),
        ...
        with self._metrics_lock:
            ...
            for name, metric_list in metrics_by_name.items():
                ...
                for metric in metric_list[-100:]:  # Last 100 values
                    ...
                    lines.append(
                        f"{name}{tags_str} {metric.value} {int(metric.timestamp * 1000)}"
                    )
