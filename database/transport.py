...
class TransportManager:
    ...
    def __init__(self, mcp_server: Server, database_manager: DatabaseManager, mcp_server_instance=None, fastmcp_server=None):
    ...
    async def run_stdio_transport(self) -> None:
        ...
        self.logger.info("TRANSPORT_STDIO_START", {
            "transport_mode": "stdio",
            "server_name": self.config.mcp.server_name
        })
        self.logger.info("TRANSPORT_STDIO_READY", {
        ...
        try:
            ...
            self.logger.info("TRANSPORT_STDIO_INTERRUPT", {
                "transport_mode": "stdio",
                "reason": "keyboard_interrupt"
            })
        except Exception as e:
            self.logger.error("TRANSPORT_STDIO_ERROR", {
                "transport_mode": "stdio",
                "error": str(e),
            ...
            try:
                ...
                self.logger.error("TRANSPORT_STDIO_SHUTDOWN_ERROR", {
                    "error": str(e),
                    "error_type": type(e).__name__
                }, exc_info=True)
            self.logger.info("TRANSPORT_STDIO_STOPPED", {
                "transport_mode": "stdio"
            })
    
    async def run_sse_transport(self, host: str, port: int) -> None:
        """Run the MCP server with SSE transport via HTTP."""
        self.logger.info("TRANSPORT_SSE_START", {
            "transport_mode": "sse",
            "host": host,
            "port": port,
            "endpoint": f"http://{host}:{port}{self.config.server.sse_path}"
        })
        self.logger.info("TRANSPORT_SSE_READY", {
        ...
        try:
            ...
            self.logger.info("TRANSPORT_SSE_INTERRUPT", {
                "transport_mode": "sse",
                "reason": "keyboard_interrupt"
            })
        except Exception as e:
            self.logger.error("TRANSPORT_SSE_ERROR", {
                "transport_mode": "sse",
                "error": str(e),
                "error_type": type(e).__name__
            }, exc_info=True)
            ...
            try:
                ...
                self.logger.error("TRANSPORT_SSE_SHUTDOWN_ERROR", {
                    "error": str(e),
                    "error_type": type(e).__name__
                }, exc_info=True)
            self.logger.info("TRANSPORT_SSE_STOPPED", {
                "transport_mode": "sse"
            })
    ...
    async def run_streamable_http_transport(self, host: str, port: int) -> None:
        ...
        try:
            ...
            logger.info("Received interrupt signal, shutting down Streamable HTTP transport")
        except Exception as e:
    ...
    def _create_sse_app(self) -> Starlette:
        ...
        async def handle_metrics(request: Request) -> Response:
            ...
            try:
                ...
                if self.mcp_server_instance and hasattr(self.mcp_server_instance, 'production_monitor'):
                    ...
                    return JSONResponse(
                        content={"error": "Monitoring not available"},
                        status_code=503
                    )
        ...
        routes = [
            ...
            Mount(self.config.server.sse_messages_path, app=sse_transport.handle_post_message)
        ]
        
        # Add health check route if enabled
        if self.config.server.enable_health_check:
            routes.append(Route(self.config.server.health_check_path, endpoint=handle_health_check))
...
class AutoTransportManager(TransportManager):
    ...
    def __init__(self, mcp_server: Server, database_manager: DatabaseManager, mcp_server_instance=None):
...
def create_transport_manager(mcp_server: Server, database_manager: DatabaseManager, mcp_server_instance=None, fastmcp_server=None) -> TransportManager:
    ...
    if config.server.transport_mode == "auto":
        ...
        return TransportManager(mcp_server, database_manager, mcp_server_instance, fastmcp_server)
