...
class SchemaAnalyzer:
    ...
    def analyze_table(self, table_info: TableInfo) -> Dict[str, Any]:
        ...
        text_columns = [col for col in table_info.columns if col.column_type in [ColumnType.TEXT, ColumnType.VARCHAR]]
        ...
        large_varchar_columns = [
            ...
            if col.column_type == ColumnType.VARCHAR and col.character_maximum_length and col.character_maximum_length > 1000
        ]
    ...
    def _calculate_table_score(self, metrics: Dict[str, Any], issues: List[Dict]) -> int:
    ...
    def analyze_database(self, schema_snapshot: SchemaSnapshot) -> Dict[str, Any]:
        ...
        for table_key, table_info in schema_snapshot.tables.items():
            ...
            overall_metrics["total_constraints"] += analysis["metrics"]["constraint_count"]
        ...
        table_scores = [analysis.get("overall_score", 0) for analysis in table_analyses.values()]
...
class SchemaComparator:
    ...
    def compare_schemas(self, schema1: SchemaSnapshot, schema2: SchemaSnapshot) -> Dict[str, Any]:
        ...
        diff = {
            ...
            "database_changes": self._compare_databases(schema1.databases, schema2.databases),
            "table_changes": self._compare_tables(schema1.tables, schema2.tables)
        }
        
        return diff
    
    def _compare_databases(self, db1: Dict[str, DatabaseInfo], db2: Dict[str, DatabaseInfo]) -> Dict[str, Any]:
        ...
        for db_name in common:
            if db1[db_name] != db2[db_name]:
                modified.append({
                    "database": db_name,
                    "changes": self._get_database_changes(db1[db_name], db2[db_name])
                })
    ...
    def _compare_tables(self, tables1: Dict[str, TableInfo], tables2: Dict[str, TableInfo]) -> Dict[str, Any]:
    ...
    def _get_database_changes(self, db1: DatabaseInfo, db2: DatabaseInfo) -> Dict[str, Any]:
        """Get specific changes between two database infos."""
        changes = {}
        
        if db1.character_set != db2.character_set:
            changes["character_set"] = {"old": db1.character_set, "new": db2.character_set}
    ...
    def _get_table_changes(self, table1: TableInfo, table2: TableInfo) -> Dict[str, Any]:
        ...
        if table1.table_comment != table2.table_comment:
            changes["comment"] = {"old": table1.table_comment, "new": table2.table_comment}
        ...
        # Compare constraints
        constraint_changes = self._compare_constraints(table1.constraints, table2.constraints)
        if constraint_changes:
            changes["constraints"] = constraint_changes
        
        return changes
    
    def _compare_columns(self, cols1: List[ColumnInfo], cols2: List[ColumnInfo]) -> Dict[str, Any]:
        ...
        return {
            ...
            "modified": modified
        } if (added or removed or modified) else {}
    
    def _compare_indexes(self, idx1: List[IndexInfo], idx2: List[IndexInfo]) -> Dict[str, Any]:
        """Compare index lists."""
    ...
    def _compare_constraints(self, const1: List[ConstraintInfo], const2: List[ConstraintInfo]) -> Dict[str, Any]:
...
class SchemaManager:
    ...
    async def get_databases(self) -> List[DatabaseInfo]:
        ...
        try:
            ...
            if not result.get("success"):
                raise DatabaseQueryError(f"Failed to get databases: {result.get('error')}")
            ...
            self.logger.error("SCHEMA_GET_DATABASES_ERROR", {
                "error": str(e),
    ...
    async def get_tables(self, database_name: Optional[str] = None, include_views: bool = False) -> List[TableInfo]:
        ...
        try:
            ...
            self.logger.error("SCHEMA_GET_TABLES_ERROR", {
                "error": str(e),
                "error_type": type(e).__name__,
    ...
    async def get_table_details(self, table_name: str, database_name: Optional[str] = None) -> TableInfo:
        ...
        try:
            ...
            for table in tables:
                if table.name == table_name and (not database_name or table.database_name == database_name):
                    table_info = table
                    break
            ...
            # Get columns
            table_info.columns = await self._get_table_columns(table_name, table_info.database_name)
            
            # Get indexes
            table_info.indexes = await self._get_table_indexes(table_name, table_info.database_name)
            
            # Get constraints
            table_info.constraints = await self._get_table_constraints(table_name, table_info.database_name)
            
            return table_info
            
        except Exception as e:
            self.logger.error("SCHEMA_GET_TABLE_DETAILS_ERROR", {
                "error": str(e),
                "error_type": type(e).__name__,
                "table_name": table_name,
                "database_name": database_name
            }, exc_info=True)
            if isinstance(e, (ValidationError, DatabaseConnectionError, DatabaseQueryError)):
                raise
            raise DatabaseConnectionError(f"Failed to retrieve table details: {str(e)}")
    
    async def _get_table_columns(self, table_name: str, database_name: str) -> List[ColumnInfo]:
        ...
        result = await self.database_manager.execute_query(query, [table_name, database_name])
        if not result.get("success"):
    ...
    async def _get_table_indexes(self, table_name: str, database_name: str) -> List[IndexInfo]:
    ...
    async def _get_table_constraints(self, table_name: str, database_name: str) -> List[ConstraintInfo]:
        ...
        result = await self.database_manager.execute_query(query, [table_name, database_name])
        if not result.get("success"):
            raise DatabaseQueryError(f"Failed to get constraints: {result.get('error')}")
    ...
    async def create_schema_snapshot(self, database_names: Optional[List[str]] = None) -> SchemaSnapshot:
        ...
        try:
            ...
            if database_names:
                databases = {db.name: db for db in all_databases if db.name in database_names}
            else:
            ...
            self.logger.error("SCHEMA_CREATE_SNAPSHOT_ERROR", {
                "error": str(e),
                "error_type": type(e).__name__,
                "database_name": database_name
            }, exc_info=True)
    ...
    async def _get_server_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            server_info = {
                "version": result.get("data", [{}])[0].get("version") if result.get("success") else None,
    ...
    def export_schema_snapshot(self, snapshot: SchemaSnapshot, format_type: str = "json") -> str:
    ...
    def _export_sql(self, snapshot: SchemaSnapshot) -> str:
        ...
        # Add header comment
        sql_statements.append(f"-- Schema export generated on {snapshot.timestamp.isoformat()}")
        sql_statements.append(f"-- MySQL version: {snapshot.mysql_version or 'Unknown'}")
        ...
        for table_key, table in snapshot.tables.items():
            if table.table_type == "BASE TABLE":  # Skip views for now
                ...
                # Add primary key
                primary_key_cols = [col.name for col in table.columns if col.is_primary_key]
                if primary_key_cols:
    ...
    async def analyze_schema(self, database_names: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error("SCHEMA_ANALYZE_ERROR", {
                "error": str(e),
                "error_type": type(e).__name__
            }, exc_info=True)
            raise DatabaseConnectionError(f"Failed to analyze schema: {str(e)}")
    
    async def compare_schemas(self, snapshot1: SchemaSnapshot, snapshot2: SchemaSnapshot) -> Dict[str, Any]:
        """Compare two schema snapshots."""
        try:
            comparison = self.comparator.compare_schemas(snapshot1, snapshot2)
            return comparison
        except Exception as e:
            self.logger.error("SCHEMA_COMPARE_ERROR", {
                "error": str(e),
                "error_type": type(e).__name__
            }, exc_info=True)
