...
class MCPConfig:
    ...
    allowed_query_types: str = (
        "SELECT, SHOW, DESCRIBE, EXPLAIN, UPDATE, INSERT, DELETE, CREATE, ALTER, DROP"
    )
    ...
    def get_allowed_query_types_list(self) -> list[str]:
        ...
        return [
            t.strip().upper() for t in self.allowed_query_types.split(",") if t.strip()
        ]
